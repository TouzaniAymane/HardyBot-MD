...
if (opcion == '1' || methodCodeQR) {
    console.log(chalk.yellow('[ ℹ️ ] يرجى مسح QR CODE بواسطة جهازك من WhatsApp.'));
 }}
  if (connection == 'open') {
    console.log(chalk.yellow('[ ℹ️ ] تم الإتصال بنجاح.'));
  }
...
if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
        conn.logger.error(`[ ⚠ ] خطأ في ملف ${global.authFile}, امسح ملف ${global.authFile} ثم اعد الإتصال مجددا.`);
        //process.exit();
    } else if (reason === DisconnectReason.connectionClosed) {
        conn.logger.warn(`[ ⚠ ] تم غلق الإتصال، حاول مجددا...`);
        await global.reloadHandler(true).catch(console.error);
    } else if (reason === DisconnectReason.connectionLost) {
        conn.logger.warn(`[ ⚠ ] تم قطع الإتصال، حاول مجددا...`);
        await global.reloadHandler(true).catch(console.error);
    } else if (reason === DisconnectReason.connectionReplaced) {
        conn.logger.error(`[ ⚠ ] تم تغيير الإتصال، حاول مجددا.`);
        //process.exit();
...
global.reloadHandler = async function(restatConn) {
  ...
  conn.welcome = '👋 ¡مرحبا!\n@user';
  conn.bye = '👋 ¡وداعا!\n@user';
  conn.spromote = '*[ ℹ️ ] @user تمت ترقيته إلى مشرف المجموعة.*';
  conn.sdemote = '*[ ℹ️ ] @user تمت إزالته من مشرف المجمعة.*';
  conn.sDesc = '*[ ℹ️ ] لقد تم تحديث وصف المجموعة.*';
  conn.sSubject = '*[ ℹ️ ] لقد تم تحديث إسم المجموعة.*';
  conn.sIcon = '*[ ℹ️ ] لقد تم تحديث صورة المجموعة.*';
  conn.sRevoke = '*[ ℹ️ ] لقد تم تحديث رابط المجموعة.*';
